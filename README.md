...
*- Note on Seeding: If using Docker, you can run `docker exec grade_api ./main` to manually trigger seed scripts if they are compiled, but standard practice is running `docker exec grade_api go run seed/seed.go` to populate your dockerized DB.*
...
1. **Install Dependencies**
   Navigate to the backend directory and download all required packages:
   ```bash
   cd backend
   go mod tidy
   ```
